...
range_of_means_gtex = subset(
  tissue_results_all,
  # TODO uncomment samples > 50 &
    mean_clr > 0 & source == "gtex" & ref_gene == 'all') %>%
  group_by(gene) %>%
...
total_tissues = length(
  unique(
    subset(
      tissue_results_all,
      # TODO uncomment samples > 50 &
      mean_clr < 0 & source == "gtex" & ref_gene == 'all')$tissue))
...
range_of_means_tcga = subset(
  tissue_results_all,
  # TODO uncomment samples > 10 &
  mean_clr > 0 & source == "tcga" & ref_gene == 'all') %>%
  group_by(gene) %>%
  summarise(iqr = iqr(rel_diff), sd = sd(rel_diff))
...
total_tissues = length(
  unique(
    subset(
      tissue_results_all,
      # TODO uncomment samples > 50 &
      mean_clr < 0 & source == "tcga" & ref_gene == 'all')$tissue))
