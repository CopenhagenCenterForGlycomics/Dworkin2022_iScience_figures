...
load.environment <- local(expr = {
  ...
  annotation.environment <- local(expr = {
    ...
    file.copy('../../input/scp/hg19v2.1.0cellRanger_gt_annotation.tsv')
  ...
  single_cell.environment <- local(expr = {
    ...
    gene_rows.df <- setNames(read.csv(file = gzfile('genes.txt.gz'), sep = '\t', header = F), c('ensembl_gene_id', 'ref_genome|gene_symbol'))
    gene_rows.df$`ref_genome|gene_symbol` <- make.unique(gene_rows.df$`ref_genome|gene_symbol`, sep = '___')
    file.remove('genes.txt.gz')
    
    # umis ----
    file.copy('../../input/scp/cellnames.umis.txt','cellnames.umis.txt')
    cell_cols.df <- setNames(read.csv(file = gzfile('cellnames.umis.txt'), sep = '\t', header = F), c('exp|method|cell|'))
    file.remove('cellnames.umis.txt')
  ...
  bulk.environment <- local(expr = {
    ...
    mixture1.bulk <- read.csv(file = gzfile('human_mixture1_genes_results.txt.gz'), sep = '\t', header = T)
    ...
    mixture2.bulk <- read.csv(file = gzfile('human_mixture2_genes_results.txt.gz'), sep = '\t', header = T)
...
count_normalization_wrapper <- function(...) {
  ...
  dump_raw_counts(
    ...
    filenames.char = 'chromium10x_replicate1_raw_umis.tsv',
    n_gene_filter.int = 500))
  
  dump_raw_counts(
      sc_mixture2.hg19.df, list(
      filenames.vec = 'chromium10x_replicate2_raw_umis.tsv', 
      n_gene_filter.int = 500))
...
sc_replicates.list <- count_normalization_wrapper(c(log1p_transform = F, sc_replicates.list))
```
