...
if(!file.exists("alldata_human_pseudopresence.Rds")){
  tsv = list.files(path='hsap_tabuladb_pseudopresence/', pattern='.gz')
  raw_alldata = sapply(paste('hsap_tabuladb_pseudopresence/', tsv, sep=''), function(x) { read.delim(gzfile(x), check.names = F) })
}
```
